...
{
#ifdef TARGET_OE_PIN
    rt_pin_write(TARGET_OE_PIN, PIN_HIGH); // disable SWD/JTAG output
#endif

    rt_pin_mode(TCK_PIN, PIN_MODE_INPUT);
#ifdef TCK_DIR_PIN
    rt_pin_write(TCK_DIR_PIN, PIN_LOW);
#endif

    rt_pin_mode(TMS_PIN, PIN_MODE_INPUT);
#ifdef TMS_DIR_PIN
...
{
    rt_pin_write(SWDIO_PIN, bit ? PIN_HIGH : PIN_LOW);
}
...
{
    rt_pin_write(TDI_PIN, bit ? PIN_HIGH : PIN_LOW);
}
...
{
    rt_pin_write(NRST_OUT_PIN, bit ? PIN_LOW : PIN_HIGH); // inverting transistor
}
